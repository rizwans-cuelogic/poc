...
def register(request):
	...
	if request.method=='POST':
		...
		if userform.is_valid() and orgform.is_valid():
		 	user = User.objects.create_user(userform.cleaned_data['username'], userform.cleaned_data['email'], userform.cleaned_data['password'],is_active='False')
	...
	try:
		if request.method=='POST':
			...
			if user.is_active:
				...
				obj=user.forgotpassword_set.create(activation_key=hash1,link_time=timezone.now())
